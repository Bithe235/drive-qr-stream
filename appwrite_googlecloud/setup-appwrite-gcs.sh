...
# Appwrite + Google Cloud Setup Script for Drive QR Stream
# This script will:
# 1. Deploy Appwrite to Google Cloud Run
...
SERVICE_NAME=${3:-"appwrite-drive-qr"}
...
# Create Cloud SQL instance for Appwrite database
...
# Create Cloud Memorystore (Redis) instance for Appwrite
...
# Create Google Cloud Storage bucket for videos
echo "Creating Google Cloud Storage bucket..."
BUCKET_NAME="${SERVICE_NAME}-videos-$(date +%s)"
...
[
  {
    "origin": ["http://localhost:5173", "https://drive-qr-stream.vercel.app", "http://localhost:8080"],
...
# Create service account for Appwrite
...
# Get connection details for Cloud SQL and Redis
echo "Getting connection details..."
DB_CONNECTION=$(gcloud sql instances describe ${SERVICE_NAME}-db --format="value(connectionName)")
REDIS_HOST=$(gcloud redis instances describe ${SERVICE_NAME}-redis --region=$REGION --format="value(host)")
...
cat > ../.env.local << EOF
VITE_APPWRITE_ENDPOINT=${SERVICE_URL}/v1
VITE_APPWRITE_PROJECT_ID=drive-qr-stream
VITE_APPWRITE_DATABASE_ID=qrcode-db
VITE_USE_GCS=true
VITE_GCS_PROJECT_ID=${PROJECT_ID}
